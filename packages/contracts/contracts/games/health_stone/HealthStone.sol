...
contract HealthStone is IBaseGame {
    IShuffleStateManager public iShuffleStateManager;
    DeckConfig _cardConfig;

    // check whether the caller is the shuffle Manager
    modifier onlyShuffleManager() {
        require(address(iShuffleStateManager) == msg.sender, "Caller is not shuffle manager.");
        _;
    }
    ...
    // a mapping between HealthStone's gameId,largestGameId=> gameId
    ...
    function newGame() external returns(uint) {
        uint256 gameId = iShuffleStateManager.createShuffleGame(2);
    ...
    // 创建者创建shuffle
    function createShuffleForCreator(uint pkX,uint pkY)external returns(uint){
      uint256 gameId = iShuffleStateManager.createShuffleGame(2);
      shuffleGameId[++largestGameId] = gameId;
      gameOwners[largestGameId] = msg.sender;
      iShuffleStateManager.playerRegister(gameId, msg.sender, pkX, pkY);
      return gameId;
    }
    ...
    function allowJoinGame(
        ...
        iShuffleStateManager.register(shuffleGameId[gameId], next);
    }
    ...
    function moveToShuffleStage(
        ...
        iShuffleStateManager.shuffle(shuffleGameId[gameId], next);
    }
    ...
    function dealCard0ToPlayer0(
        ...
        iShuffleStateManager.dealCardsTo(
    ...
    function openCard0(
        ...
        iShuffleStateManager.openCards(
            shuffleGameId[gameId],
            0,
            1,
            next
    ...
    function endGame(
        ...
        iShuffleStateManager.endGame(gameId);
    }
}